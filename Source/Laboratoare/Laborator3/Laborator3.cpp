...
void Laborator3::Init()
{
	...
	Mesh* orbitSquare = Object2D::CreateSquare("orbitSquare", corner, orbitSide, glm::vec3(1, 0, 1));
	AddMeshToList(orbitSquare);
}
...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	scaleX += deltaTimeSeconds * 0.09;
    scaleY += deltaTimeSeconds * 0.09;
    angularStep += deltaTimeSeconds * 0.01;
	...
	//BONUS: Un patrat care orbiteaza in jurul altuia

	orbitMatrix = glm::mat3(1);
	

	float gravityCenterX = modelMatrix[0][0] + modelMatrix[1][0] + modelMatrix[2][0] + cx;
	float gravityCenterY = modelMatrix[0][1]+ modelMatrix[1][1]+modelMatrix[2][1] + cy;
	
	orbitMatrix *= Transform2D::Translate(gravityCenterX, gravityCenterY);
	orbitMatrix *= Transform2D::Rotate(angularStep);
	orbitMatrix *= Transform2D::Translate(-gravityCenterX, -gravityCenterY);
	orbitMatrix *= Transform2D::Translate(100, 100);
	orbitMatrix *= modelMatrix;
	
	RenderMesh2D(meshes["orbitSquare"], shaders["VertexColor"], orbitMatrix);


	modelMatrix = glm::mat3(1);
	gravityCenterX = 400 + cx;
	gravityCenterY = 250 + cy;
	modelMatrix *= Transform2D::Translate(gravityCenterX, gravityCenterY);

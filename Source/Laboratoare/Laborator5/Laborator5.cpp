...
void Laborator5::Init()
{
	...
	fov = 60.0f;
	left,top = 0;
	bottom = window->props.resolution.y / 100;
	right = window->props.resolution.x / 100;
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_W)) {
			...
			camera->TranslateForward(offset	* cameraSpeed);
		}

		if (window->KeyHold(GLFW_KEY_A)) {
			camera->TranslateRight(-offset * cameraSpeed);
		}
		...
		if (window->KeyHold(GLFW_KEY_Q)) {
			...
			camera->TranslateUpword(offset * cameraSpeed);
		}
	...
	if (window->KeyHold(GLFW_KEY_Z)	&& isPerspective) {

		fov -= offset;
		projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, 0.01f, 200.0f);
	}
	else if (window->KeyHold(GLFW_KEY_Z) && !isPerspective) {
		left -= offset; right += offset; bottom -= offset; top += offset;
		projectionMatrix = glm::ortho(right, left, top, bottom, 0.01f, 200.0f);
	}
	if (window->KeyHold(GLFW_KEY_X)	&& isPerspective) {

		fov += offset;
		projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, 0.01f, 200.0f);
	}
	else if (window->KeyHold(GLFW_KEY_X) && !isPerspective) {
		left += offset; right -= offset; bottom += offset; top -= offset;
		projectionMatrix = glm::ortho(right, left, top, bottom, 0.01f, 200.0f);
	}

...
void Laborator2::Init()
{
	...
	{
		vector<VertexFormat> cubeVertices {
		...
		vector<VertexFormat> tetraVertices {
			...
			VertexFormat(glm::vec3(1,0,0), glm::vec3(1,0,0)),
			VertexFormat(glm::vec3(0,2,0), glm::vec3(1,0,0))
		...
		vector<unsigned short> cubeIndices =
		{
			...
			6,3,0,
			6,0,4
			// cubeIndices for second triangle
			// TODO: Complete cubeIndices data
		};
		vector<unsigned short> tetraIndices = {
			0,1,3,
			3,1,2,
			2,1,0,
			3,2,0
		};
		vector <unsigned short> squareIndices = {
			0,2,1,
			1,3,2
			
		};
		meshes["cube1"] = new Mesh("generated cube 1");
		meshes["cube1"]->InitFromData(cubeVertices, cubeIndices);

		// Create a new mesh from buffer data
		CreateMesh("cube3", cubeVertices, cubeIndices);
		
		CreateMesh("tetra", tetraVertices, tetraIndices);
...
Mesh* Laborator2::CreateMesh(const char *name, const std::vector<VertexFormat> &cubeVertices, const std::vector<unsigned short> &cubeIndices)
{
	unsigned int VAO = 0;
	...
	// TODO: Send cubeVertices data into the VBO buffer
	//STATIC MEANS THE DATA IS MODIFIED ONCE AND USED MANY TIMES
	glBufferData(GL_ARRAY_BUFFER, sizeof(cubeVertices[0]) * cubeVertices.size(), &cubeVertices[0], GL_STATIC_DRAW);
	...
	// TODO: Send cubeIndices data into the IBO buffer
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(cubeIndices[0]) * cubeIndices.size(), &cubeIndices[0], GL_STATIC_DRAW);
	// ========================================================================
	...
	//vreau sa trimit 3 floaturi, gl_false este normalizat(daca trb sa aiba lungime 1), distanta de cand am citit un elem pana la urma
	...
	meshes[name]->InitFromBuffer(VAO, static_cast<unsigned short>(cubeIndices.size()));
	return meshes[name];
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(2, 1, -2));
	RenderMesh(meshes["tetra"], shaders["VertexColor"], glm::vec3(-2, 0, -1));
...
void Laborator2::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_F2) {
		if (cullFace == GL_FRONT) cullFace = GL_BACK;
